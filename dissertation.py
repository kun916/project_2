...
merged_df['Datetime'] = pd.to_datetime(merged_df['Datetime'],dayfirst=True)
...
is_data = round_numeric_columns(is_data)
bs_data = round_numeric_columns(bs_data)
cf_data = round_numeric_columns(cf_data)
ratios = round_numeric_columns(ratios)
metrics_df = round_numeric_columns(metrics_df)
company_basic_data = round_numeric_columns(company_basic_data)
...
def clean_and_convert_to_numeric(df, exclude_columns):
    for col in df.columns:
        if col not in exclude_columns:
            # Remove any non-numeric characters, except for the negative sign and decimal point
            df[col] = df[col].replace(r'[^\d.-]', '', regex=True)
            # Convert to numeric, forcing errors to NaN
            df[col] = pd.to_numeric(df[col], errors='coerce')
    return df
val_ratios = clean_and_convert_to_numeric(val_ratios, exclude_columns=['Datetime', 'symbol'])
merged_df = clean_and_convert_to_numeric(merged_df, exclude_columns=['Datetime', 'symbol'])
